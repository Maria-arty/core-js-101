...
function reverseInteger(num) {
  const newStr = num.toString().split('').reverse().join('');
...
function getDigitalRoot(num) {
  let currentNum = num;
  while (currentNum > 9) {
    currentNum = currentNum.toString().split('').reduce((acc, el) => acc + parseInt(el, 10), 0);
  }
  return currentNum;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (open.includes(bracket)) {
      ...
      const index = close.indexOf(bracket);
...
function toNaryString(num, n) {
  ...
  let ourNum = num;
  while (ourNum > 0) {
    newNum.unshift(ourNum % n);
    ourNum = Math.floor(ourNum / n);
  }
  return newNum.join('');
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < firstPath.length; i += 1) {
    const checkelem = firstPath[i];
    if (pathes.every((path) => path.split('/')[i] === checkelem)) {
      common.push(checkelem);
    } else {
      break;
    }
  }
